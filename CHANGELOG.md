...
- **`certificates.signCSR` — issue an end-entity certificate from a caller-built
  CSR (§27.1, §27.5, contract 1.45).** `POST /api/v1/certificates/sign-csr`,
  generated by `Scripts/gen_management.py` from the vendored registry like every
  other §27 operation. Takes a PEM PKCS#10 request for a key AXIAM never holds
  and answers the existing `Certificate` — **not** `GeneratedCertificate`, whose
  `privateKeyPEM` is mandatory and would always be absent here. No key crosses
  the wire in either direction; a model round-trip test asserts the decoded
  response structurally carries no private-key field at all, and contrasts it
  with `GeneratedCertificate`, which legitimately does. The management surface
  moves 159 → 160 operations across the same 24 namespaces — the generated
  conformance suite's own count assertion was regenerated along with it.

- **`webauthnSetupRegisterStart` / `webauthnSetupRegisterFinish` — a passkey or
  security key as the first factor at forced first-login enrolment (§24.1,
  §24.5, §24.7, §24.8, §25.1, §25.2, contract 1.45).**
  `POST /api/v1/auth/webauthn/setup/register/{start,finish}`, the WebAuthn twin
  of `mfaSetupEnroll` / `mfaSetupConfirm`: a tenant that requires MFA no longer
  limits a new user's first factor to a TOTP app.

  Both calls take **no session** — the setup token from `login`'s `403` is the
  only credential, and it travels in the body. Unlike `webauthnRegisterStart` /
  `webauthnRegisterFinish`, neither requires a session client-side, and neither
  attaches this client's own session credential, even when one happens to be
  configured: a new `AxiamClient.setupTokenRawSend(path:body:)` transport is
  used in place of `webauthnRawSend`, which still emits the §5 tenant header and
  still captures a `Set-Cookie` / `X-CSRF-Token` the response sets (so
  `setupRegisterFinish` can complete the interrupted login), but never sends a
  `Cookie` header of its own. A test asserts on the transport that neither call
  carries the session's cookie even with one genuinely configured.

  `webauthnSetupRegisterFinish` adopts credentials **exactly as
  `mfaSetupConfirm` does** — it mirrors that function's implementation, clearing
  the §17 decision memo and adopting the session through the same
  `LoginSuccessResponse` → `AxiamUser` path, because both are the completion of
  the same interrupted login and a caller must end up authenticated the same way
  regardless of which factor the user chose. A `403` from `setupRegisterFinish`
  surfaces the tenant's attestation-policy message verbatim, exactly as
  `webauthnRegisterFinish` already does.

  `setupToken` is `Sensitive<String>`, per this SDK's existing convention for
  every bearer credential named in the contract.

  On iOS 16+ / macOS 13+, `webauthnSetupRegister(setupToken:credentialName:anchor:attachment:)`
  additionally composes the new pair with a ceremony, mirroring `webauthnRegister`
  exactly (same `WebauthnCeremony`, same guards). It is not one of §24.1's three
  named composed helpers — the contract does not canonicalize a fourth for the
  setup pair — but is added anyway since it invents no new authenticator API and
...
- **CONTRACT.md §10.4 — an optional session-revocation feed poller (contract
  1.44).** The new `RevocationFeed` actor, switched on with
  `AxiamConfig.revocationFeedEnabled` (and paced by `revocationPollInterval`),
  plus `RevocationFeed.entry(for:)` and the three bounds the contract names.
  ...
  Nothing changes unless you enable it: the flag defaults to `false`, and when
  it is `false` **no poller is constructed at all** — "default off" is a
  structural property here rather than a branch taken at verification time. It
  never fetches on the request path after the first call; `authenticate` reads a
  cached set. And it never fails closed: an unreachable feed, a non-`200`, an
  unparseable body or an unknown `alg` all behave exactly as no feed at all, and
  specifically **not** as an empty list, which would assert that nothing has
  been revoked. A token with no `sid` is never matched against it, and there is
  no fallback to `jti`. The feed is an `actor`, so one instance serves every
  guard built from a client.

- `JwtClaims.sid`, decoded from the OIDC Core `sid` claim; `nil` when the token
  carries none, which keeps "no session behind this token" distinct from "a
  session named the empty string".
...
- **A malformed `mtls_endpoint_aliases` entry now throws instead of falling back
  ...
  The refusal is an `AxiamError.auth`, not `.network`: nothing failed in
  ...
  leaves every other endpoint working, and a client built without
  `clientCertificate` never reads the member at all.
...
- **RFC 8705 §5 `mtls_endpoint_aliases` (SDK contract 1.40, CONTRACT.md §21.3
  ...
  property (the new `MtlsEndpointAliases`), and the §12 operations now prefer an
  alias over the top-level entry of the same name on any call made over mutual
  TLS — that is, from a client carrying a `clientCertificate`. Five operations
  reach the token endpoint (`oidcExchange`, `oidcRefresh`,
  `loginClientCredentials`, `devicePoll`, `tokenExchange`), plus `introspect`,
  `revoke`, `deviceAuthorize` and `oidcPar`.

  A `nil` property means "this deployment terminates mutual TLS on the issuer's
  ...
  synthesised for `authorizationEndpoint`, `endSessionEndpoint` or `jwksURI`,
  which are front-channel or public. `issuer` does not move, and §12.4 rule 3
  still compares a token's `iss` against it by exact string — including for a
  token minted at an alias endpoint.

  The UMA 2.0 ticket grant is deliberately untouched: it reads its token
  endpoint from the separate `Uma2Configuration` document, which carries no
