...
extension AxiamClient {
    ...
    public func introspect(
        ...
        let document = try await oidcConfiguration(configuration)
        guard let endpoint = try preferredEndpoint(document, { $0.introspectionEndpoint }, document.introspectionEndpoint) else {
    ...
    public func revoke(
        ...
        let document = try await oidcConfiguration(configuration)
        guard let endpoint = try preferredEndpoint(document, { $0.revocationEndpoint }, document.revocationEndpoint) else {
            throw AxiamError.network(NetworkError(
